...
async def test_client_disconnect_mid_transaction(bouncer):
    ...
    # Check for the log message indicating cleanup
    with bouncer.log_contains(r"cleaning up transaction after client disconnect"):
        pass
    ...
    p1_final_count = 0
    # Handle both string and list return types
    lines = final_pool_info.splitlines() if hasattr(final_pool_info, 'splitlines') else final_pool_info
    for line in lines:
        if isinstance(line, str) and line.startswith('p1'):
    ...
    assert p1_final_count >= p1_initial_count, "Server connection was not returned to the pool"
    
    # Make a new connection to verify the pool is still working
    async with bouncer.acur(dbname="p1") as cur:
        await cur.execute("SELECT 1")
        result = await cur.fetchone()
        assert result[0] == 1, "Failed to execute query after client disconnect scenario"
